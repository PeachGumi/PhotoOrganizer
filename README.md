...
ボリュームの識別情報に加え、論理ボリューム → パーティション → 物理ディスクの対応を追跡します。`MSFT_Disk.BusType` も確認し、VHDなどの仮想ディスク、Storage Spaces、物理デバイスを一意に確認できない構成を独立した保存先と認めません。

### macOS

Disk Arbitrationからボリュームやパーティションの識別情報を取得し、IOKitの親子関係をたどって元になっている物理ディスクを確認します。APFSの論理コンテナを別の物理ディスクと誤認しないよう、実際のブロックデバイスまで確認します。

ディスクイメージやRAMディスク、物理デバイスとの対応が曖昧な構成は、独立した保存先として認めません。実機テストでは `diskutil info -plist` の情報と照合し、APFSの場合は `APFSPhysicalStores` の実ディスクまで確認します。
...
│ macOS: Disk Arbitration / IOKit / LaunchAgent│
└─────────────────────────────────────────────┘
```
...
実際のmacOSランナーではネイティブAPIの識別結果を `diskutil` と照合し、使い捨てのディスクイメージが独立した物理デバイスと認められないことも確認します。照合に使うplistの項目欠落・型違い・優先順位・壊れたXMLなどは単体テストで確認します。
